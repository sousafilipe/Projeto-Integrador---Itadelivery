...
class ClienteModel:
    ...
    def cadastrar_cliente(self, nome, cpf, telefone, genero, senha):
        conn = self.conectar_banco()
        cursor = conn.cursor()

        try:
            cursor.execute("INSERT INTO clientes (nome, cpf, telefone, genero, senha) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                           (nome, cpf, telefone, genero, senha))
